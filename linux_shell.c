...
#include <dirent.h>
#include <fcntl.h>
#include <sys/utsname.h> // NEW: For sysinfo command
...
// NEW: Prototypes for the new custom functions
...
void update_styles(AppContext *ctx)
{
    ...
    // Recreate tags for dark/light color sync
...
void toggle_theme_cb(GtkToggleButton *button, AppContext *ctx)
{
    ...
    // 🔁 Update toggle button label accordingly
...
// MODIFIED: Added checks for new built-in commands
gboolean handle_builtin(AppContext *ctx, int argc, char *args[])
{
    if (argc <= 0)
        return FALSE;
    ...
    if (strcmp(args[0], "cd") == 0)
    {
        ...
        if (chdir(dir) != 0)
        {
            ...
            snprintf(error_msg, sizeof(error_msg), "Error: Could not change directory to '%s': %s\n", dir, strerror(errno));
    ...
    // NEW: Dispatchers for new commands
...
gboolean builtin_countdown(AppContext *ctx, int argc, char *args[])
{
    ...
    {
        char buf[64];
        snprintf(buf, sizeof(buf), "Time left : %d\n", i);
...
// NEW: Implementation of the `pwd` command
gboolean builtin_pwd(AppContext *ctx, int argc, char *args[])
{
...
// NEW: Implementation of the `history` command
gboolean builtin_history(AppContext *ctx, int argc, char *args[])
{
    for (guint i = 0; i < ctx->history->len; i++)
    {
        const char *cmd = g_ptr_array_index(ctx->history, i);
...
// NEW: Implementation of the `sysinfo` command
gboolean builtin_sysinfo(AppContext *ctx, int argc, char *args[])
{
...
// NEW: Implementation of the recursive helper for `search`
void search_recursive(AppContext *ctx, const char *base_path, const char *pattern, int *match_count)
{
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        // Process UI events to keep the GUI responsive during long searches
        while(gtk_events_pending()) {
            gtk_main_iteration();
        }

        // d_type is not guaranteed, so fall back to g_file_test if needed
        gboolean is_dir = (entry->d_type == DT_DIR);
        if (entry->d_type == DT_UNKNOWN) { // Filesystem doesn't provide type info
...
// NEW: Implementation of the `search` command
gboolean builtin_search(AppContext *ctx, int argc, char *args[])
{
    if (argc < 2)
    {
...
void on_app_activate(GApplication *app, gpointer user_data)
{
    ...
    gtk_window_set_title(GTK_WINDOW(ctx->window), "GTK Shell");
    gtk_window_set_default_size(GTK_WINDOW(ctx->window), 800, 600);
    // Note: The context should be freed when the application QUITS, not just when the window is destroyed.
    // We will leave the original free logic in main via g_object_unref on the app.
    // If you need to save history on exit, a "shutdown" signal on the application is better.

    // Header Bar and Menu
    GtkWidget *header = gtk_header_bar_new();
    gtk_header_bar_set_show_close_button(GTK_HEADER_BAR(header), TRUE);
    gtk_header_bar_set_title(GTK_HEADER_BAR(header), "GTK Shell");
    ...
    GtkWidget *theme_toggle = gtk_toggle_button_new_with_label("Dark Mode");
    ...
    gtk_text_buffer_create_tag(ctx->buffer, "prompt", "foreground", ctx->is_dark_theme ? "#87CEFA" : "#0000CD", "weight", PANGO_WEIGHT_BOLD, NULL);
    gtk_text_buffer_create_tag(ctx->buffer, "error", "foreground", ctx->is_dark_theme ? "#FF6347" : "#DC143C", "weight", PANGO_WEIGHT_BOLD, NULL);
    gtk_text_buffer_create_tag(ctx->buffer, "highlight", "foreground", ctx->is_dark_theme ? "#F0E68C" : "#DAA520", NULL);
    gtk_text_buffer_create_tag(ctx->buffer, "center",
                               "justification", GTK_JUSTIFY_CENTER,
                               "foreground", ctx->is_dark_theme ? "#F0E68C" : "#DAA520",
    ...
    gchar *capitalized = g_strdup(username);
    if (g_ascii_islower(capitalized[0]))
        capitalized[0] = g_ascii_toupper(capitalized[0]);
    gchar *welcome_msg = g_strdup_printf("Welcome %s\n", capitalized);
    append_text(ctx, welcome_msg, "center");
    g_free(welcome_msg);

    append_text(ctx, "HorizonShell Initialized. Type your commands below.\n\n", "center");
...
{
    // Each time the program is run, a new context is created for it.
    AppContext *ctx = app_context_new();

    // G_APPLICATION_NON_UNIQUE allows multiple, separate instances of the app to run.
    GtkApplication *app = gtk_application_new("com.github.user.gtkshell", G_APPLICATION_NON_UNIQUE);

    // The context is passed to the activate signal, which sets up the window.
    g_signal_connect(app, "activate", G_CALLBACK(on_app_activate), ctx);

    int status = g_application_run(G_APPLICATION(app), argc, argv);

    g_object_unref(app);
    // NEW: We created ctx, so we must free it when the app is done.
